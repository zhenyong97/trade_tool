...
class MPL_Panel_Base(wx.Panel):
    ...
    def __init__(self,parent):  
        ...
        gs = gridspec.GridSpec(4, 1, left=0.1, bottom=0.10, right=0.96, top=0.96, hspace=0.1, height_ratios=[3.5,1,1,1])
    ...
    def draw_subgraph(self,stockdat,numt):  
        ...
        mpf.candlestick2_ochl(self.am, stockdat.Open, stockdat.Close, stockdat.High, stockdat.Low,width=0.5, colorup='r', colordown='g')#绘制K线走势
        ''' 绘制均线 '''    
    ...
    def draw_jumpgap(self,stockdat,jump_pd):  
        ...
        for kl_index in np.arange(0, jump_pd.shape[0]):
            today = jump_pd.iloc[kl_index]#若版本提示已经弃用 可使用loc或iloc替换
...
class Loop_Panel_Base(wx.Panel):
    ...
    def __init__(self,parent):  
        ...
        gs = gridspec.GridSpec(3, 1, left=0.1, bottom=0.10, right=0.96, top=0.96, wspace=None, hspace=0.1, height_ratios=[1.5,1,1])

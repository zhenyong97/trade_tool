...
class MPL_Panel_Base(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        gs = gridspec.GridSpec(4, 1, left=0.1, bottom=0.10, right=0.96, top=0.96, hspace=0.1,
    ...
    def draw_subgraph(self, stockdat, numt):
        ...
        mpf.candlestick2_ochl(self.am, stockdat.Open, stockdat.Close, stockdat.High, stockdat.Low, width=0.5,
        ...
        ''' 绘制成交量 '''
        self.vol.bar(numt, stockdat.Volume, color=['g' if stockdat.Open[x] > stockdat.Close[x] else 'r' for x in
    ...
    def draw_jumpgap(self, stockdat, jump_pd):
        ...
        for kl_index in np.arange(0, jump_pd.shape[0]):
            today = jump_pd.iloc[kl_index]  #
            inday = stockdat.index.get_loc(jump_pd.index[kl_index])
            if today['jump_power'] > 0:
                self.am.annotate('up', xy=(inday, today.Low * 0.95), xytext=(inday, today.Low * 0.9),
                                 arrowprops=dict(facecolor='red', shrink=0.1), horizontalalignment='left',
                                 verticalalignment='top')
            elif today['jump_power'] < 0:
                self.am.annotate('down', xy=(inday, today.High * 1.05), xytext=(inday, today.High * 1.1),
                                 arrowprops=dict(facecolor='green', shrink=0.1), horizontalalignment='left',
                                 verticalalignment='top')
...
class Loop_Panel_Base(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        gs = gridspec.GridSpec(3, 1, left=0.1, bottom=0.10, right=0.96, top=0.96, wspace=None, hspace=0.1,
    ...
    def xylabel_tick_lim(self, title):
        ...
        # self.profit.xaxis.set_minor_locator(mdates.WeekdayLocator(byweekday=(1),interval=2))
        # self.profit.xaxis.set_minor_formatter(mdates.DateFormatter('%Y %m %d'))#标签设置为日期

        # self.profit.grid(True, c='k')
        # self.total.grid(True, c='k')
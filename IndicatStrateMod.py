...
class Excave_Indic_Base:
    ...
    def plot_Ndays_Break(self, stock_df):
        ...
        for kl_index in np.arange(0, stock_df.shape[0]):
            today = stock_df.iloc[kl_index]
            """ 收盘价超过N2最低价 卖出股票持有"""
            if today['Close'] < today['N2_Low']:
                break_pd = break_pd.append(today)
                dispCont_List.append("向下突破:" + stock_df.index[kl_index].strftime('%Y-%m-%d') + ',' + str(
                    today['Close']) + '\n')  # 向下突破和价格
            """ 收盘价超过N1最高价 买入股票持有"""
            if today['Close'] > today['N1_High']:
                break_pd = break_pd.append(today)
                dispCont_List.append("向上突破:" + stock_df.index[kl_index].strftime('%Y-%m-%d') + ',' + str(
                    today['Close']) + '\n')  # 向上突破和价格
...
class QuantPickTimeSys:
    ...
    def init_buy_factors(self, buy_factors):
        ...
        for factor_class in buy_factors:
            ...
            if not isinstance(factor, FactorBuyAverBreak) and not isinstance(factor,
                                                                             FactorBuyNdayBreak):  # 判断factor为基于FactorBuyBreak实例
    ...
    def init_sell_factors(self, sell_factors):
        ...
        for factor_class in sell_factors:
            ...
            if not isinstance(factor, FactorSellAverBreak) and not isinstance(factor,
                                                                              FactorSellNdayBreak):  # 判断factor为基于FactorBuyBreak实例
    ...
    def run_factor_plot(self, subplotP0, subplotP1, subplotP2):
        ...
        list_signal = []
        is_win = False
        self.kl_pd['Ma30'] = self.kl_pd.Close.rolling(window=30).mean()  # 增加M30移动平均线
        ...
        for kl_index, today in self.kl_pd.iterrows():
            ...
            if signal > 0:  # 买入
                if is_win == False:  # 空仓则买
                    ...
                    self.cash_hold = 0
                    dispCont_List.append("买入时间:"  + "     |" + " 价格\n" + self.kl_pd.index[start].strftime('%Y-%m-%d') + ' | ￥ ' + str(
                        round(today.Close, 2)) + '\n')

                    subplotP0.annotate('B', xy=(kl_index, self.kl_pd.Close.asof(kl_index)),
                                       xytext=(kl_index, self.kl_pd.Close.asof(kl_index) + 4),
                ...
                if is_win == True:  # 避免未买先卖
                    ...
                    is_win = False
                    dispCont_List.append("卖出时间:" + "     |" + " 价格\n" + self.kl_pd.index[end].strftime('%Y-%m-%d') + ' | ￥ ' + str(
                        round(today.Close, 2)) + '\n')
                    self.cash_hold = int(self.posit_num * today.Close)
                    self.market_total = 0

                    if self.kl_pd.Close[end] < self.kl_pd.Close[start]:  # 赔钱显示绿色
                        subplotP0.fill_between(self.kl_pd.index[start:end], 0, self.kl_pd.Close[start:end],
                                               color='green', alpha=0.38)

                    else:  # 赚钱显示绿色
                        subplotP0.fill_between(self.kl_pd.index[start:end], 0, self.kl_pd.Close[start:end], color='red',
                                               alpha=0.38)

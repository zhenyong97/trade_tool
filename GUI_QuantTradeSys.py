...
from text.RedefPanelMod import MPL_Panel_Base,Loop_Panel_Base
from text.StockDataMod import GetStockDatPro, format_code_name
from text.IndicatStrateMod import Excave_Indic_Base, QuantPickTimeSys, FactorPickStockAng
...
class Frame(wx.Frame):   
    def __init__(self):   
       ...
       #策略选取
        ...
        self.StratInputBox = wx.RadioBox(self.ParaPanel, -1, label='指标提示', choices=RadioList,majorDimension = 4, style = wx.RA_SPECIFY_ROWS)
    ...
    def PstockpEvent(self,event):
        ...
        # print(type(dlg.GetValue()))
        # self.stockName_CMBO = wx.ComboBox(self.ParaPanel, -1,self.stockName_Val, choices = list(self.StNameCodedict.keys()), style = wx.CB_DROPDOWN) #股票名称

            # self.stockName_CMBO.Update()
            # self.stockName_CMBO.Refresh()
            # self.ParaPanel.Refresh()
            self.stockName_CMBO.Append(stockName)
            self.stockName_CMBO.Update()
            self.stockName_CMBO.Refresh()
